...
C10_HOST inline int clz(uint32_t x) {
  return __builtin_clz(x);
}
#if defined(__HCC__) || defined(__HIPCC__) || defined(__CUDA_ARCH__)
C10_DEVICE inline int clz(uint32_t x) {
  return __clz(x);
}
#endif

template <int we, int wm, typename T, bool negative_zero_nan, bool clip>
C10_HOST_DEVICE uint8_t
fp8_fnuz_from_fp32_value(T _x, bool stoch, uint32_t rng) {
#if defined(__HCC__) || defined(__HIPCC__)
...
inline C10_HOST_DEVICE T fp8_fnuz_to_fp32_value(uint8_t x) {
#if defined(__HCC__) || defined(__HIPCC__)
  constexpr bool is_half = std::is_same<T, _Float16>::value;
#else
  constexpr bool is_half = false;
